<?php  
	...
	$query_credito = "SELECT CONCAT(cliente.nombres,' ',cliente.apellidos) AS cliente,
					DATE(ctacorriente_cliente.fecha) AS fecha,
					CONCAT(comprobante_tipo.comprobante_tipo,' ', comprobante.serie, comprobante.serie) AS comprobante,
					producto.producto,
					ctacorriente_cliente_env.cantidad,
					FORMAT(ctacorriente_cliente_env.cantidad * producto.precio, 2) AS monto_envases,
					ctacorriente_cliente.monto AS monto_contenido,
					FORMAT((ctacorriente_cliente_env.cantidad * producto.precio) + ctacorriente_cliente.monto, 2) AS monto_total
					FROM ctacorriente_cliente , almacen , cliente , ctacorriente_cliente_env , producto , ventas , comprobante , comprobante_tipo
					WHERE almacen.almacen_id = ctacorriente_cliente.almacen_id 
					AND cliente.cliente_id = ctacorriente_cliente.cliente_id
					AND ctacorriente_cliente.ctacorriente_cliente_id = ctacorriente_cliente_env.ctacorriente_cliente_id
					AND ctacorriente_cliente_env.producto_id = producto.producto_id
					AND ctacorriente_cliente.ctacorriente_cliente_id = ventas.ventas_id
					AND comprobante.comprobante_tipo_id = comprobante_tipo.comprobante_tipo_id
					AND ventas.condicion_pago = comprobante.comprobante_id
					AND ctacorriente_cliente.almacen_id = 2
					GROUP BY ctacorriente_cliente_env.cantidad
					ORDER BY cliente.cliente_id ASC " ;
	mysql_select_db($database_fastERP, $fastERP);
						...
						<div class="bloque col-xs-4 col-sm-3">
							<table class='table table-condensed'>
								...
								<thead>
									...
									<th>Cliente</th>
									<th>producto</th>
									<th nowrap>Envases</th>
									<th nowrap>Envases (S/.)</th>
									<th nowrap>Contenido (S/.)</th>
									<th nowrap>Total (S/.)</th>
								</thead>
								<tbody>
									<?php do { ?>
										<tr>
											...
											<td nowrap><?php echo $row_credito['producto']; ?></td>
											<td nowrap class="text-center"><?php echo $row_credito['cantidad']; ?></td>
											<td nowrap class="text-center"><?php echo $row_credito['monto_envases']; ?></td>
											<td nowrap class="text-center"><?php echo $row_credito['monto_contenido']; ?></td>
											<td nowrap class="text-center"><?php echo $row_credito['monto_total']; ?></td>
										</tr>
